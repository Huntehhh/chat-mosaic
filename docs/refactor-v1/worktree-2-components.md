...
## Backend Integration (from backend-services session)

### 12. Wire ProcessManager into Extension (Task 6.1)
- **Location:** `src/extension.ts` lines 457-620
...
- **Prerequisites:** ✅ `src/types/process.ts`, ✅ `ProcessManager.ts` updated, ✅ `async-mutex` installed

### 13. Extract MCP Code to McpService (Task 6.2)
- **Location:** `src/extension.ts` lines 1779-2671 → `src/services/McpService.ts`
- Move `_initializeMCPConfig()` → `McpService.initialize()`
- Move `_loadMCPServers()` → `McpService.loadServers()`
- Move `_saveMCPServer()` → `McpService.saveServer()`
- Move `_deleteMCPServer()` → `McpService.deleteServer()`
- **Prerequisites:** ✅ `src/services/McpService.ts` shell with interfaces and atomic write pattern
...
| ProcessManager Integration | backend | High | async-mutex |
| McpService Extraction | backend | High | - |
